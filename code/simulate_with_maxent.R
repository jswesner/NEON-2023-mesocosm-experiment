...
# get mass-metabolism b's from Meng's maxent models and from our empirical data

maxent_b = read_delim("data/maxent_b.txt", delim = "|")[-1,c(-1, -5)] %>% mutate_if(is.character,as.numeric) %>% 
  clean_names()

b_mod = readRDS("models/brm_metab.rds")
dat = b_mod$data
b_posts = dat %>% 
...
empirical_b = b_posts %>% 
  ...
  rename(b_empirical = slope)

all_b = maxent_b %>% left_join(empirical_b %>% select(tank, b_empirical)) %>% 
  pivot_longer(cols = -tank) %>% 
  filter(!is.na(value)) 
...
lambda_sims = all_b %>% rename(gamma = value) %>% 
...
tte_plot_2 = lambda_sims %>% 
  ...
  stat_density_2d(geom = "polygon", data = lambda_b %>% filter(.draw <= 1000), aes(x = b_epred, y = .epred, 
                         ...
                         alpha = 0.3) +
  geom_point(aes(color = 10^log10te)) +
  labs(x = "Metabolic Scaling Exponents (empirical or maxent)",
       y = "\u03bb (ISD Exponent)",
       color = "TTE",
       caption = "Dots show predicted lambdas as a function of TTE, PPMR, and metabolic scaling.\n
       The contours show the uncertainty in empirical lambdas and metabolic scaling exponents.") +
  guides(fill = "none")
